        ...
        <div v-if="user">
            ...
            <div v-else>
                ...
                <div class="container campaign">
                    ...
                    <div class="center">
                        <router-link :to="{name: $t('informations.dealer_info_all_url')}" class="infos_link_css">
                            {{$t('homepage.infos_link_des')}}
                        </router-link>
                    </div>
                        ...
                        <div v-for="info in dealer_informations" :key="info.id" class="body-pannel col-md-4 col-sm-12">
                            <div class="pannel">
                                <div class="head">
                                    <h5>
                                        <router-link
                                            :to="{name: 'information_detail_en', query: {id: info.id, author: info.name}}">
                                            {{info.title != null && info.title.length>40 ? info.title.slice(0,40)+'...'
                                            : info.title}}
                                        </router-link>
                                    </h5>
                                </div>
                                ...
                                <div class="content">
                                    <h6>{{info.content != null && info.content.length>80 ?
                                    info.content.slice(0,80)+'...' : info.content}}</h6>
                                </div>
                                <div class="read">
                                    <h6>
                                        <router-link
...
export default {
    ...
    methods: {
        async getDealerInformations(lang) {
            var new_info_dealer_datas = await axios.get('/api/get/new_informations/dealer/' + lang + '/few')
