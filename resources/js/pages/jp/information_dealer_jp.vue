<template>
    <div>
        ...
        <div class="container content">
            <div class="no_datas" v-if="dealer_informations.length == '0'">
                <h5>新しい情報はありません</h5>
            </div>
            <div v-else class="datas row">
                <div v-for="info in dealer_informations" :key="info.id" class="pannels col-md-3 col-sm-12">
                    <div class="pannel">
                        <div class="info_title">
                            <h4>
                                <router-link
                                    :to="{name: 'information_detail_jp', query: {id: info.id, author: info.nikename?info.nikename:info.name}}">
                                    {{info.title != null && info.title.length>80 ? info.title.slice(0,80)+'...' :
                                    info.title}}
                                </router-link>
                            </h4>
                        </div>
                        <div class="info_date">
                            <h6>{{info.created_at}}</h6><br />
                            <h6>著者: <a href="#"><i>{{ info.nikename ? info.nikename : info.name }}</i></a></h6>
                        </div>
                        <div class="info_content">
                            <h5>{{info.content != null && info.content.length>120 ? info.content.slice(0,120)+'...' :
                            info.content}}</h5>
                        </div>
            ...
            <div v-if="user && user.role=='admin'" class="info_post_content">
                <div class="new_info_form">
                    ...
                    <form @submit.prevent="new_information_post" method="post">
                        ...
                        <div class="mb-3 row">
                            <div class="col-md-12">
                                <label class="col-form-label">コンテンツ</label>
...
export default {
    ...
    methods: {
        async getInformations(lang) {
            var new_info_dealer_datas = await axios.get('/api/get/new_informations/dealer/' + lang + "/all")
