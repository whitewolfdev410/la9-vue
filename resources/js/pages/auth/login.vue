            ...
            <div class="col-md-7">
              <div style="display: flex">
                <input v-model="form.password" :class="{ 'is-invalid': form.errors.has('password') }"
                  class="form-control" :type="passwordType" name="password">
                <b-icon v-if="passwordType=='password'" icon="eye-fill" class="border rounded password-display"
                  @click="passwordTypeChange"></b-icon>
                <b-icon v-else icon="eye-slash-fill" class="border rounded password-display"
                  @click="passwordTypeChange"></b-icon>
              </div>
  ...
  methods: {
    ...
    redirect() {
      ...
      if (intendedUrl) {
        ...
        this.$router.push({ path: intendedUrl }).catch(err => {
        ...
        this.$router.push({ name: this.locale }).catch(err => {
          // Ignore the vuex err regarding  navigating to the page they are already on.
          if (
            err.name !== 'NavigationDuplicated' &&
            !err.message.includes('Avoided redundant navigation to current location')
