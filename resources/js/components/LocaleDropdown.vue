<template>
  <li v-if="Object.keys(locales).length > 1" class="nav-item dropdown bottom" @mouseover="languageOver"
    @mouseleave="languageLeave">
    <a v-if="mobile" class="nav-link" href="#" role="button" data-bs-toggle="dropdown" aria-haspopup="true"
      aria-expanded="false" style="display: inline">
      {{ $t('language') }}
    </a>
    <a v-else class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-haspopup="true"
      aria-expanded="false">
      {{ $t('language') }}
    </a>
