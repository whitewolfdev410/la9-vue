<template>
    <div class="container comment">
            ...
            <b-card v-for="comment in comments_data" :key="comment.id">
                <template #header>
                    <div class="text-md-start" style="display:flex;">
                        ...
                        <h5 class="mb-0 text-md-start" style="padding-left: 10px;">{{comment.nikename ? comment.nikename
                        : comment.name }}</h5>
                    </div>
                    <div class="text-md-end">
                        <b-button @click="reply_param(comment.info_id, comment.id)"
                            v-b-modal="'modal_reply_'+comment.info_id+'_'+comment.id" variant="link">{{$t('reply')}}
                        </b-button>
                        <b-button @click="view_reply_action(comment.id)"
                            v-b-modal="'modal_view_'+comment.info_id+'_'+comment.id" variant="link">
                            {{$t('answers_view')}}</b-button>
                        <b-button v-if="user && user.id == comment.user_id"
                            @click="edit_comment_param(comment.info_id, comment.id, comment.comment)"
                            v-b-modal="'modal_edit_'+comment.info_id+'_'+comment.id" variant="link">{{$t('edit')}}
                        </b-button>
                        <b-button v-if="user && user.id == comment.user_id"
                            @click="delete_comment_action(comment.info_id, comment.id)" variant="link">{{$t('delete')}}
                        </b-button>

                        <!-- Modal part -->
                        <!-- reply modal -->
                        <b-modal hide-footer :id="'modal_reply_'+comment.info_id+'_'+comment.id"
                            :ref="'modal_reply_'+comment.info_id+'_'+comment.id" :title="$t('reply')">
                            <form @submit.prevent="reply_post" method="post">
                                <div class="col-md-12">
                                    <div class="body-pannel">
                                        <b-form-textarea type="text" v-model="reply" rows="3" ref="reply" required>
                                        </b-form-textarea>
                                    </div>
                                </div>
                        ...
                        <!-- view reply modal -->
                        <b-modal hide-footer scrollable :id="'modal_view_'+comment.info_id+'_'+comment.id"
                            :ref="'modal_view_'+comment.info_id+'_'+comment.id" :title="$t('all_answers')">
                            ...
                            <div v-else>
                                <b-card v-for="reply in reply_data" :key="reply.id" style="padding: 5px 0;">
                                    <b-card-body>
                                        <div class="text-md-start" style="display:flex;">
                                            <img :src="'/upload/users/'+reply.photo_url"
                                                class="rounded-circle profile-photo me-1">
                                            <h6 class="mb-0 text-md-start" style="padding-left: 10px;">
                                                {{reply.comment_answer}}</h6>
                                        </div>
                                    </b-card-body>
                                </b-card>
                            </div>
                        ...
                        <!-- edit comment modal -->
                        <b-modal hide-footer :id="'modal_edit_'+comment.info_id+'_'+comment.id"
                            :ref="'modal_edit_'+comment.info_id+'_'+comment.id" :title="$t('edit')">
                            <form @submit.prevent="edit_comment_post" method="post">
                                <div class="col-md-12">
                                    <div class="body-pannel">
                                        <b-form-textarea type="text" v-model="edit_comment_content" rows="3" required>
                                        </b-form-textarea>
                                    </div>
                                </div>
                                <div class="col-md-12 col-sm-12" style="padding-top: 5px;">
        ...
        <div v-if="user">
            <form @submit.prevent="comment_post" method="post">
                <div class="col-md-12">
                    ...
                    <div class="body-pannel">
                        <b-form-textarea type="text" v-model="comment" rows="5" ref="comment"></b-form-textarea>
                    </div>
                </div>
                <div class="col-md-12 col-sm-12" style="padding-top: 5px;">
                    <div class="body-pannel" style="text-align:right;">
        ...
        <div v-else>
            <h6>In order to edit new comment, Please <router-link :to="{name: 'login'}">{{ $t('login') }}</router-link>
            </h6>
        </div>
