<template>
  <div class="container information">
    <div class="title">
      ...
      <router-link :to="{name: $t('informations.url')}" class="infos_link_css">{{$t('homepage.infos_link_des')}}
      </router-link>
    </div>
    <div class="body row">
...
export default {
  ...
  methods: {
    async getInformations(lang) {
      try {
        var new_info_datas = await axios.get('/api/get/new_informations/' + lang + '/' + this.user.role + '/few')
        this.informations = new_info_datas.data
      } catch (error) {
        var new_info_datas = await axios.get('/api/get/new_informations/' + lang + '/' + "notLogin" + '/few')
        this.informations = new_info_datas.data
      }
    }
  },
